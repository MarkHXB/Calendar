...
namespace Calendar
{
    ...
    { 
        ...
        private void DeleteTaskBtn_Click(object sender, EventArgs e)
        {
            ...
            int selectedTaskIndex = ConvertTo.Int(parent.Name);
            ...
            {
                if (item.Task_Id_Dates == Calendar.selectedTask[selectedTaskIndex - 1].Task_Id_Dates)
                {
                    //set DB row value
        ...
        private int SelectRowByID(int selectedTask)
        {
            ...
            foreach (var item in Task.DbContent)
            {
                if (item.Task_Id_Dates == Calendar.selectedTask[selectedTask - 1].Task_Id_Dates)
                {
                    output = counter;
                }
                counter++;
